...
**Deliberately NOT touched** (these were labeled "kapsam dışı" / out-of-scope in the plan, not "ertelenen" / deferred — bigger infra work, not a quick fix): real FCM push notifications for parking alerts, notification dedup/history, AndroidManifest App Links host correction + Firebase Hosting `assetlinks.json` publishing. If the user meant these too, ask before starting — they require new server-side infrastructure and a deploy, which CLAUDE.md requires explicit request for.
