...
3. **Firestore collection/rules mismatch — NOT YET FIXED.** Client code writes to `bikes`, `friendships`, `lobbies`, `parking_notifications`, `rides` (plus `sync_coordinator.dart`'s own `users/{uid}/{entityType}` subcollection model). `firestore.rules` only allows `users`, `public_rider_cards`, `rider_tags`, `entitlements`, `notification_tokens/*/devices/*`, `bug_reports`(+subs); everything else hits a catch-all `allow read, write: if false`. So friends, group ride lobbies, and parking notifications are unreachable in the cloud today. Root cause for #5 below.
...
5. **Account deletion doesn't fully clear Firestore — NOT YET FIXED, root-caused by #3.** `deleteUserAccount()` in `firebase_service.dart` attempts `bikes`/`rides`/`friendships` deletes filtered by `userId`, but those collections are blocked by the rules mismatch in #3 — the delete queries will silently fail (`catch (_) {}` swallows the permission-denied error). Also never attempts `notification_tokens/{uid}/devices/*`, `lobbies`, or `parking_notifications`.
